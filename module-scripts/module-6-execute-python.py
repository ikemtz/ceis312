...
def azureml_main(frame1):
    ...
    fig.clf()
    ax = fig.gca()
    temp0.plot(kind='scatter', x='PC1', y='PC2', color='DarkBlue',
               label='Group 0', alpha=0.3, ax=ax)
    temp1.plot(kind='scatter', x='PC1', y='PC2', color='Red',
               label='Group 1', alpha=0.3, ax=ax)
    ...
    for col in num_cols:
        ...
        temp0.plot(kind='scatter', x=col, y='area', color='DarkBlue',
                   label='Group 0', alpha=0.3, ax=ax)
        temp1.plot(kind='scatter', x=col, y='area', color='Red',
                   label='Group 1', alpha=0.3, ax=ax)
        ax.set_title(title)
        ax.set_xlabel('')
        i += 1
    ...
    if (Azure == True):
        fig.savefig('Scatter_vs_area.png')
    ...
    for col in num_cols:
        ...
        temp0.plot(kind='scatter', x=col, y='FFMC', color='DarkBlue',
                   label='Group 0', alpha=0.3, ax=ax)
        temp1.plot(kind='scatter', x=col, y='FFMC', color='Red',
                   label='Group 1', alpha=0.3, ax=ax)
        ax.set_title(title)
        ax.set_xlabel('')
        i += 1
    ...
    if (Azure == True):
        fig.savefig('Scatter_vs_FFMC.png')
